...
test("trivial inline numeric math remains lossless and idempotent in prose prompts", () => {
  const amc10 = JSON.parse(fs.readFileSync("data/amc10.json", "utf8"));
  ...
  assert.equal(sanitizeForMathJax(out), out);
});
...
test("GRE OCR lim notation is repaired without discarding a trailing number", () => {
  const input = "Evaluate: limx→0+ (sinx x )1/x^2. 25";
  const out = sanitizeForMathJax(input);
  assert.equal(out, "Evaluate: $\\lim_{x\\to 0+} (\\frac{\\sin x}{x})1/x^2. 25$");
  assert.equal(sanitizeForMathJax(out), out);
});
